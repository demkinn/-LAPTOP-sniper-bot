import { createPublicClient, createWalletClient, http, parseEther, formatEther, getAddress, erc20Abi, type Address, type Hash } from 'viem';
import { base } from 'viem/chains';
import { privateKeyToAccount } from 'viem/accounts';
import { config } from './config.js';
import type { Quote } from './zerox.js';
import { quoteGasGwei, quoteImpactBps, NATIVE_ETH } from './zerox.js';
...
export function getAccountAddress(): Address { return account().address; }
...
async function sendQuote(q: Quote): Promise<Hash> {
  ...
  const tx: { to: Address; data: `0x${string}`; value?: bigint; gas?: bigint; gasPrice?: bigint } = {
...
export async function executeNativeBuy(q: Quote): Promise<Hash> { return sendQuote(q); }

export async function ensureAllowance(token: Address, spender: Address, amount: bigint): Promise<Hash | null> {
  if (token.toLowerCase() === NATIVE_ETH.toLowerCase()) return null;
...
export async function executeTokenSell(q: Quote): Promise<Hash> {
  ...
  if (token.toLowerCase() === NATIVE_ETH.toLowerCase()) throw new Error('executeTokenSell called with native ETH.');
  const spenderRaw = q.issues?.allowance?.spender || q.allowanceTarget;
  if (!spenderRaw) throw new Error('0x returned no explicit allowance target; refusing to approve any contract.');
